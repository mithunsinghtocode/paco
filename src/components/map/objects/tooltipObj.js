...
export const tooltipObj = (line, lineSeries, am4core, flight, displayView, index) => {
  ...
  flight.arrStn === 'SIN' ? (bullet.tooltipHTML = `<div style="margin-bottom:0px;margin-top:0px;color:#fff;width:auto;height:22px;font-family:Proxima Nova Regular;font-size: 14px;margin-right:5px;" onHover="cursor: pointer">
      <b style="font-family:Proxima Nova Bold">${getFltNum(flight)}</b> ${displayLine()} ${getETA(flight)} ${displayLine()} <b  style="font-family:Proxima Nova Bold;margin-right:2px">${getTotMisconnectedPax(flight, displayView)}</b>
    </div>`) : bullet.tooltipHTML = `<div style="margin-bottom:0px;margin-top:0px;color:#fff;width:auto;height:22px;font-family:Proxima Nova Regular;font-size: 14px;">
